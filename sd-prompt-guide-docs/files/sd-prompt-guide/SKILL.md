...
description: |
  Stable Diffusion (Illustrious XL 等 SDXL 系) + Tsubaki / Mio 等 DiT 系
  兩大架構的 Prompt 實戰指南。包含 Danbooru tag 資料庫、可協助使用者查詢
  有效的 tag 和中英對照。

  ⚠️ 第一步：先判斷使用者用哪一架構
  - SDXL 系（Illustrious、NoobAI、Pony 等）→ Danbooru tag 寫法（本 SKILL 主路線）
  - DiT 系（Tsubaki、Tsubaki.2、Mio.2、Serin 等）→ 自然英文敘述（跳到 references/basics/16b-dit-prompt-tips.md）

  兩條路線寫法完全不同、不要混用。

  通用功能：
  (1) 協助使用者撰寫 prompt（任何主題）
  (2) 查詢 Danbooru tag 是否存在、使用次數、別名（僅 SDXL）
  (3) 中英對照查詢
  (4) 模型推薦
  (5) 常見實用小技巧
  ...
  (14b) DiT 模型 Prompt 撰寫指南（自然語言寫法）
  (14c) Tsubaki.2 使用心得 / 自訂風格功能 / Style Code 分享
...
|------|---------|-----------|-----------|
| **SDXL** | Illustrious-XL、NoobAI、Pony、ChocoMint Mix 等 | Danbooru 標籤語言（`1girl, black_hair, smile`） | **本 SKILL 主路線**：查 tag 庫驗證、給 tag 組合 |
| **DiT** | Tsubaki、Tsubaki.2、Tsubaki Flash、Serin、Mio.2 等 | 自然英文敘述（"A young girl with black hair smiling..."） | **跳到 [16b-dit-prompt-tips.md](references/basics/16b-dit-prompt-tips.md)**：不查 tag 庫、不需要 1girl/1boy 等 fix tag |

**判斷方式**：
- 使用者明說「Tsubaki / Mio / DiT 模型」→ DiT 路線
- 使用者明說「Illustrious / NoobAI / Pony」→ SDXL 路線
- 使用者沒講、但 prompt 寫了 `1girl, masterpiece` 之類標籤 → 預設 SDXL
- 使用者寫了長段自然英文（"A girl in a cafe..."）→ 預設 DiT
- 不確定就**問一下使用者**

**重要**：DiT 路線跟本 SKILL 後面寫的「Tag 驗證」「Danbooru 資料庫查詢」「底線轉空白」**通通不適用**。如果使用者用 DiT、請走 16b 那份檔、不要硬套 SDXL 流程。
...
| 我想要... | 參考文件 |
|-----------|----------|
| **DiT 模型 Prompt 寫法（必讀）** | `references/basics/16b-dit-prompt-tips.md` |
| 找適合的 DiT 模型 | `references/basics/16-recommended-dit-models.md` |
| Tsubaki.2 使用心得 | `references/basics/17-tsubaki2-review.md` |
| 自訂風格功能介紹 | `references/basics/18-style-code-intro.md` |
| Style Code 分享 | `references/basics/19-style-code-sharing.md` |
...
> 🔔 **適用範圍**：本段適用於 SDXL 系（Illustrious、NoobAI、Pony 等）。
> DiT 系（Tsubaki、Mio、Serin 等）**不需要**驗證 tag 存在、改用自然英文敘述、詳見 `references/basics/16b-dit-prompt-tips.md`。
